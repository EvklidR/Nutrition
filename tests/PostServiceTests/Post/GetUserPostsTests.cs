...
{
    public class GetUserPostsTests
    {
        ...
        [Fact]
        public async Task PostService_ShouldRetutnUserPosts()
        {
            //Arrange
            var posts = _postFaker.Generate(5);
            var userId = _faker.Random.AlphaNumeric(24);

            _postRepositoryMock.Setup(pr => pr.GetByUserIdAsync(userId, It.IsAny<int?>(), It.IsAny<int?>()))
                .ReturnsAsync((posts, posts.Count));

            //Act
            var response = await _postService.GetUserPostsAsync(1, 5, userId);

            //Assert
            response.TotalCount.Should().Be(posts.Count);
            response?.Posts?.Select(x => x.Id).Should().BeEquivalentTo(posts.Select(x => x.Id));

            _postRepositoryMock.Verify(pr => pr.GetByUserIdAsync(userId, It.IsAny<int?>(), It.IsAny<int?>()), Times.Once);
        }
    }
}
